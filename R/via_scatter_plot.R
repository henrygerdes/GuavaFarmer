...
#'
#' @param guava_data guava data file
#' @param Cell.Line cell line to analyse
#' @param Drugs drugs to analyse
#' @param Metric metric for graph
#' @param Method methof for line of best fit
#'
#' @return a ggplot element
#' @export plot_via_scatter
...
plot_via_scatter <- function(guava_data, Cell.Line, Drugs, Metric, Method){
  ...
  df_graph <-
    guava_data[(guava_data$Cell.Line %in% Cell.Line) &
                 (guava_data$Drug %in% c(Drugs, "DMSO")), c("SampleID", "Cell.Line", "Drug", "Concentration", Metric)]

  colnames(df_graph) <-
    c("SampleID", "Cell.Line", "Drug", "Concentration", "Metric")
  ...
  guava_calc <- foreach(i = Drugs, .combine = "rbind") %do% {
    ...
    out <-
      out[, c("mean.Concentration", "mean.Metric", "sd.Metric")]#
  ...
  pp <-
    ...
    geom_pointrange(data = guava_calc,
                    aes(
                      x = Concentration,
                      ymin = Mean - SD,
                      ymax = Mean + SD,
    ...
    scale_x_continuous(trans = "log1p",
                       breaks = unique(guava_calc$Concentration)) +
  ...
  if (Method == "nls") {
    ...
    conc_pred <-
      log1p(seq(0, max(guava_calc$Concentration), length.out = 100))

    guava_lines <- foreach(i = Drugs, .combine = "rbind") %do% {
      df <- guava_calc[guava_calc$Drug == i, ]
      bfl <-
        nls(data = df,
            formula = metric_norm ~ SSlogis(log_conc, Asym, xmid, scal))
      pred <-
        data.frame((exp(conc_pred) - 1), (predict(bfl, conc_pred) * (max(df$Mean) +
                                                                       max(df$SD))))
      pred$Drug <- i
    ...
    pp <-
      pp + geom_line(data = guava_lines,
                     aes(x = Concentration, y = Metric_1, col = Drug),
                     size = 2)
  } else{
    pp <- pp + geom_smooth(method = Method,
                           size = 2,
                           se = F)
  }
